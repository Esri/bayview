define([
    ...
    function (Map, SpatialReference, EsriUtils,
        webMercatorUtils, Polygon, Point, Extent, Graphic,
        ...
        return {
            ...
            createMap: function (divId, config) {
                ...
                if (divId && config) {
                    ...
                    mapObject = new this._mapObject(divId, config.map.options, config);
            ...
            createWebMap: function (webMapId, divId, options) {
                var mapObject,
                    deferredOut;
                if (divId && options) {
                    mapObject = new this._mapObject(divId, options);
            ...
            initTopics: function(mapObj) {
                var map = mapObj._mapObject._map;
                ...
                topic.subscribe("/map/zoom/extent", lang.hitch(this, function(sender, args) {
                    //var wkid = (args.wkid != null) ? parseInt(args.wkid, 10) : this.wkid;
                    var wkid = parseInt(args.wkid, 10);
                ...
                topic.subscribe("/map/zoom/geometry", lang.hitch(this, function(sender, geometry) {
                    ...
                    if (geometry) {
                      ...
                      if (geom.type === "point") {
                          ...
                          var extendChanged = map.setExtent(geom.getExtent());
                          extendChanged.then(function(evt) {
                            console.log("map zoomed to extent geometry", evt);
                          });
                      }
                    } else {
            ...
            _mapObject: declare(null, {
                constructor: function (mapDivId, options, config) {
                    ...
                    this._layerController = new LayerController();
                },
                ...
                createMap: function () {
                    ...
                    this._operationalLayers = this._config.operationalLayers;
                ...
                createWebMap: function (webMapId) {
                    ...
                    getDeferred = function (response) {
                        ...
                        this._bindEvents();
                        this._mapDiv.__map = this._map;
                        this._mapObject = myselfAsMapObject;
                        this._mapObject._setMapDiv(true);
                        this._map._mapObject = this._mapObject;

                        this._operationalLayers = response.itemInfo.itemData.operationalLayers;
                        this._initMapWidgets();
                ...
                _initMapWidgets: function() {
                    ...
                    this._layerController.setMap(this._map);
                    this._layerController.addOperationalLayers(this._operationalLayers);
                ...
                _bindEvents: function () {
                    ...
                    if (this._map.loaded) {
                        ...
                        //this._handles.push(on(this._map, "update-end", lang.hitch(this, selectionChange)));
                    }

                    this._map.infoWindow.on("selection-change", lang.hitch(this, function(e){
                        topic.publish("/InfoWindow/selectionChanged", { target: e.target });
                    }));
                ...
                _setMapDiv: function (forceResize) {
                    ...
                    if (this._visible) {
                        ...
                        domStyle.set(this._mapDivId, {
                            "height": this._mapDiv.parentNode.clientHeight + "px",
                            "width": "auto"
                        });
